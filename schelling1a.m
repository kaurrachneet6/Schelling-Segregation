%Rachneet Kaur
%No switching agents
function [contact susep] = schelling1a(lattice,dummylattice,toler,l_size)
figure
disp(lattice);
pause(100)
    ...
    for i=1:l_size
        for j=1:l_size
            ...
            if ( visit(x,y) == 0 && lattice(x,y) ~= 0)
                ...
                if (fr > toler)
                    if ( rand(1)< u_move )
                        [newx,newy] = moveto(lattice,x,y,lattice(x,y),toler);
                        if (newx ~= 0)
                            visit(x,y) = 1;
                            visit(newx,newy) = 1;
                            tmp = lattice(x,y);
...
function [nx,ny] = moveto(lattice, x, y, color, delta)
nx = 0;
ny = 0;
flag = 0;
o = 1;
l_size= length(lattice);
omax = abs(max([(x-l_size),(l_size-x),(y-l_size),(l_size-y)]));
while (~flag)
