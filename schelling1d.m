%Rachneet Kaur
%Active switching agents ( activated after half time )
function [contact susep] = schelling1d(lattice,dummylattice,toler,l_size)
figure
disp(lattice);
max = 500;
u_move = 0.2;
...
title('Segregation model with switching agents activated after half time');
pause(0.015);
end
...
for k=1:8
    ngbr_r = x + neigh(k, 1);
    ngbr_c = y + neigh(k, 2);
    if (in(lattice, ngbr_r, ngbr_c))
        if (lattice(ngbr_r,ngbr_c) ~= 0)
            count = count+1;
            if (lattice(ngbr_r,ngbr_c) == 1)
                agent1 = agent1 + 1;
            end
        end
    end
